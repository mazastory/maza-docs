...
import { 
  ...
  Plus, AlertTriangle, X, Eye, Send, Loader2, Star, ChevronRight, Cpu, FileText
} from "lucide-react";
...
export default function KnowledgeHub({ 
  ...
  return (
    <div className={`${isIntegrated ? '' : 'max-w-7xl mx-auto py-16 px-6'} space-y-16`}>
      ...
      {/* Interactive Standalone Visual Manual Banner */}
      <div className="bg-gradient-to-r from-indigo-950 via-slate-900 to-indigo-950 border border-indigo-500/20 p-8 rounded-[40px] shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden group">
        <div className="absolute top-0 right-0 p-8 opacity-[0.03] text-8xl font-black italic pointer-events-none select-none">LIVE GUIDE</div>
        <div className="flex items-center gap-5">
          <div className="w-14 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center shrink-0 shadow-lg group-hover:scale-105 transition-transform duration-300">
            <Globe2 size={28} className="text-indigo-400" />
          </div>
          <div className="text-left">
            <div className="flex items-center gap-2">
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-500 animate-ping" />
              <span className="text-[10px] font-black text-indigo-300 tracking-[0.2em] uppercase">FULLSCREEN INTERACTIVE MANUAL</span>
            </div>
            <h3 className="text-xl font-black text-white italic tracking-tight uppercase mt-1">
              지식 허브가 복잡하신가요? <span className="text-indigo-400">100% 시각 매뉴얼 전체화면 모드</span>로 시작해보세요!
            </h3>
            <p className="text-[11px] text-slate-400 font-bold mt-1 max-w-2xl leading-relaxed">
              복잡한 탭 화면을 벗어나 가독성을 극대화한 전용 뷰어입니다. 크롬 익스텐션 1초 설치, 도메인 연결 CNAME 설정, GA4 "andrew" 바이패스 설정 과정을 초고화질 스크린샷 가이드로 감상하세요.
            </p>
          </div>
        </div>
        <button 
          onClick={() => navigate('/installation-guide')}
          className="px-6 py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-xl shadow-indigo-600/20 flex items-center gap-2 shrink-0 group-hover:shadow-[0_0_30px_rgba(79,70,229,0.4)]"
        >
          시각화 매뉴얼 보러가기 <ArrowRight size={14} />
        </button>
      </div>

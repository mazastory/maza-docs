...
router.get('/stats', requireAuth, async (req, res) => {
  try {
    ...
    if (seoData && seoData.length > 0) {
      const total = seoData.reduce((acc: number, curr: any) => acc + (curr.seo_score || 0), 0);
...
router.get('/hall-of-fame', async (req, res) => {
  try {
    ...
    const approvedSites = (data || []).filter((s: any) => 
       s.adsense_status === 'approved' || s.is_setup_complete === true
    );
...
router.post('/clusters', requireAuth, async (req, res) => {
  try {
    ...
    if (!site_id || !name) return res.status(400).json({ success: false, error: 'site_id와 name은 필수입니다.' });
...
router.get('/clusters/:id', requireAuth, async (req, res) => {
  try {
    ...
    const realTimeScore = await AuthorityEngine.calculateTopicalDepth(cluster.site_id as string, id as string);

...
import { getScopedClient } from "../lib/supabaseServer.js";
...
router.post("/setup-ga4", requireAuth, async (req, res) => {
  try {
    ...
    if (!domain) return res.status(400).json({ error: "도메인 주소가 필요합니다." });
    if (!providerToken) return res.status(401).json({ error: "Google 권한 토큰이 필요합니다." });
    ...
    if (!property || !property.name) throw new Error("GA4 속성 생성에 실패했습니다.");
...
router.post("/setup-search-console", requireAuth, async (req, res) => {
  try {
    ...
    const siteVerification = (google as any).siteVerification({ version: 'v1', auth });
...
router.get("/sitemap-status", requireAuth, async (req, res) => {
  try {
    ...
    const auth = getOAuth2Client();
    auth.setCredentials({ access_token: providerToken as string });

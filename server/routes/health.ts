...
export async function getSystemStats() {
  ...
  return {
    ...
    db: {
      ...
      success24h: recentPosts?.filter((p: { status: string }) => ['published', 'draft', 'scheduled'].includes(p.status)).length || 0,
      failed24h: recentPosts?.filter((p: { status: string }) => p.status === 'failed' || p.status === 'error').length || 0,
      retry24h: recentPosts?.filter((p: { status: string }) => p.status === 'retry').length || 0
    },
    budget: {

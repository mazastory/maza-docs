...
export class ObservabilityService {
   ...
   * 워커 메트릭 기록 (각 워커에서 호출)
   */
  static async recordMetric(workerName: string, jobId: string, status: 'success' | 'failed', startTime: number, errorType?: string) {
    try {
  ...
  private static processFailureHeatmap(data: any[]) {
    // 24시간을 1시간 단위로 그룹화

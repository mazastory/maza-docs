 ...
 * 단일 포스트 생성 및 저장 엔진
 */
export async function generateAndSavePost(userId: string, siteId: string, keyword: string) {
 ...
 */
export async function generateFromExperience(userId: string, siteId: string, imageBase64: string, keyword: string) {
  if (!supabaseAdmin) throw new Error('Supabase Admin client not initialized');
  const { VisionAgent } = await import('./visionAgent.js' as any);
  const { uploadBufferToR2 } = await import('./imageService.js' as any);
  ...
  const uploadResult = await uploadBufferToR2(buffer, 'image/jpeg', data.title) as any;
 ...
 * 시리즈 벌크 스케줄링 엔진
 */
export async function scheduleSeries(userId: string, seriesId: string) {

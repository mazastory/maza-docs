...
export interface RenderData {
  title?: string;
  intro?: string;
  authorBox?: { name?: string; bio?: string; description?: string; avatar?: string; image?: string; };
  image1?: string;
  content1?: string;
  insightBox?: string;
  summary?: string;
  image2?: string;
  definitionSection?: { title: string; content: string; };
  comparisonData?: { title: string; headers: string[]; rows: string[][]; };
  practicalExamples?: { case: string; bad: string; good: string; }[];
  content2?: string;
  experienceNote?: string;
  image3?: string;
  faq?: { q?: string; question?: string; a?: string; answer?: string; }[];
  detailedFaqs?: { q?: string; question?: string; a?: string; answer?: string; }[];
  outro?: string;
  hashtags?: string[];
 ...
 */
export function applyHTMLTemplate(data: RenderData, topic: string, options?: { adsenseStatus?: string; adsensePub?: string; coupangId?: string; coupangTracking?: string; }): string {
  ...
  const headings = [];
  if (data.content1) headings.push({ id: "toc-1", text: `${topic}의 핵심 원리` });
  if (data.definitionSection) headings.push({ id: "toc-2", text: data.definitionSection.title });
  if (data.comparisonData) headings.push({ id: "toc-3", text: data.comparisonData.title });
  if (data.content2) headings.push({ id: "toc-4", text: "심층 가이드" });
  if (faqs.length > 0) headings.push({ id: "toc-5", text: "자주 묻는 질문" });
  ...
  // 2. 광고 블록 (지능형 멀티 배치)
  const getAdBlock = (slot: string) => isApproved ? `
...
<div style="max-width:750px; margin:0 auto; font-family:'Pretendard','Inter',-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif; color:#1e293b; line-height:1.9; word-break:keep-all; background-color:#ffffff; padding:20px;">
    ...
    <table style="width:100%; border-collapse:separate; border-spacing:0; border-radius:24px; overflow:hidden; border:1px solid #f1f5f9; font-size:16px; box-shadow:0 10px 30px rgba(0,0,0,0.02);">
      <thead>
        <tr style="background:#f8fafc;">
          ${data.comparisonData.headers.map((h: string) => `<th style="padding:22px; border-bottom:1px solid #f1f5f9; text-align:left; font-weight:900; color:#475569;">${h}</th>`).join("")}
        </tr>
      </thead>
      <tbody>
        ${data.comparisonData.rows.map((row: string[], rIdx: number) => `
  ...
  <div style="margin:60px 0;">
    ...
    <div style="display:grid; gap:30px;">
      ${data.practicalExamples.map((ex: any) => `
  ...
  <div style="margin-top:50px; padding:40px; background:linear-gradient(to bottom, #f8fafc, #ffffff); border-radius:40px; border:1px solid #e2e8f0; box-shadow:0 15px 40px rgba(0,0,0,0.03);">
    ...
    <div style="display:grid; gap:12px;">
       ${data.metadata.series.posts.map((p: any) => `
  ...
  <div style="margin-top:70px; padding-top:40px; border-top:1px solid #f1f5f9; display:flex; flex-wrap:wrap; gap:10px;">
    ${data.hashtags.map((t: string) => `<span style="background:#f8fafc; color:#64748b; padding:8px 18px; border-radius:100px; font-weight:700; font-size:14px; border:1px solid #e2e8f0;">#${t.replace('#', '')}</span>`).join("")}
  </div>` : ""}
...
export class RendererAgent {
  static renderPost(data: RenderData, options: { 
    postId?: string; 
    day?: number; 
    topic: string; 
    ...
    adsensePub?: string; 
    coupangId?: string; 
    coupangTracking?: string;
    internalLinks?: { title: string; url: string; }[];
  }): RenderResult {
    if (!data) return { title: options.topic, blocks: [], html: "" };
    ...
    // 2. 인트로 및 요약
    if (data.intro) blocks.push({ type: "intro", text: data.intro });
    if (data.summary) blocks.push({ type: "summary", text: data.summary });

    // 3. 메인 이미지 및 섹션 1
    if (data.image1) blocks.push({ type: "image", url: data.image1, alt: title });
    if (data.content1) {
      blocks.push({ type: "heading", text: `${options.topic}의 핵심 원리와 이해`, level: 2 });
      blocks.push({ type: "paragraph", text: data.content1 });
    }

    if (data.insightBox) blocks.push({ type: "insight", text: data.insightBox });
    if (data.summary) blocks.push({ type: "summary_box", text: data.summary });

    // 4. 섹션 2 (분석 및 정의)
    if (data.image2) blocks.push({ type: "image", url: data.image2, alt: `${options.topic} 상세` });
    ...
    if (data.practicalExamples) {
      ...
      data.practicalExamples.forEach((ex: any) => {
    ...
    if (data.experienceNote) blocks.push({ type: "experience", text: data.experienceNote });
    if (data.image3) blocks.push({ type: "image", url: data.image3, alt: `${options.topic} 마무리` });

    // 6. FAQ 및 결론
    if (faqs.length > 0) blocks.push({ type: "faq", items: faqs });
    if (data.outro) blocks.push({ type: "conclusion", text: data.outro });
    if (data.hashtags) blocks.push({ type: "hashtags", items: data.hashtags });
...
export function renderBlocksToHTML(blocks: any[], title?: string): string {
  ...
  if (title) html += `<h1 style="font-size:28px; font-weight:800; margin-bottom:20px;">${title}</h1>`;
  blocks.forEach(block => {
    const text = block.text || block.content || "";
    switch (block.type) {
      case "heading": html += `<h2 style="font-size:22px; font-weight:700; margin:30px 0 10px;">${text}</h2>`; break;
      case "paragraph": html += `<p style="font-size:16px; line-height:1.8; margin-bottom:20px;">${text}</p>`; break;
      case "image": html += `<img src="${block.url}" style="width:100%; border-radius:20px; margin:25px 0;" alt="${block.alt || ""}" />`; break;

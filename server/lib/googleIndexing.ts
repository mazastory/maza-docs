...
export class GoogleIndexingService {
  private static auth = new google.auth.JWT({
   ...
   */
  static async requestIndexing(url: string, type: 'URL_UPDATED' | 'URL_DELETED' = 'URL_UPDATED') {
   ...
   */
  static async getStatus(url: string) {
    try {
      const indexing = google.indexing({
        version: 'v3',
        auth: this.auth

...
export function initWebSocketServer(httpServer: any) {
  ...
  setInterval(async () => {
    try {
      const { getSystemStats } = await import('../routes/health.js' as any);
...
export function waitForResult(jobId: string, timeoutMs: number = 30000) {
...
function handleActionResult(data: any) {
...
function handleActionError(data: any) {
  const handler = resultHandlers.get(data.jobId);
  if (handler) {
    clearTimeout(handler.timer);
    resultHandlers.delete(data.jobId);

...
// ──────────────────────────────────────────────────────────
// Step 1: Parse markdown to plain HTML (default renderer)
// Step 2: Inject inline CSS via regex post-processing
// This approach is renderer-version-agnostic.
// ──────────────────────────────────────────────────────────
...
function injectStyles(html) {
  return html
    // Wrapper font (applied externally)
    ...
    // Pre code block (needs special treatment: first fix pre, then re-fix code inside pre)
    ...
    // Override code inside pre to have transparent background
    .replace(/(<pre[^>]*>)\s*(<code)[^>]*(>)/g, (match, pre, codeTag, gt) =>
      `${pre}<code style="background:transparent;color:inherit;padding:0;font-family:ui-monospace,SFMono-Regular,monospace;font-size:14px;white-space:pre;"${gt}`
    )

    // Links
    .replace(/<a href=/g, `<a style="color:${PRIMARY};text-decoration:underline;font-weight:600;" href=`)
...
const outputDir = path.join(baseDir, 'export_html');
...
function processDirectory(directory) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      if (file !== 'export_html' && file !== 'node_modules') {
      ...
      // 1. Convert markdown → plain HTML using default marked renderer
      const rawHtml = marked.parse(cleanContent);

      // 2. Inject inline CSS styles
      const styledHtml = injectStyles(rawHtml);

      // 3. Wrap in container
      const wrappedHtml =
        `<div style="max-width:800px;margin:0 auto;font-family:${FONT};padding:20px;">\n` +
        styledHtml +
        `\n</div>`;

...
| `server/lib/taskOrchestrator.ts` | **(핵심)** 익스텐션에 넘겨줄 때 티스토리 실제 예약 시간(`target_publish_at`)을 최근 히스토리 기반으로 동적 계산. <br> 1) **2시간 Fast Track**: 마지막 발행으로부터 2시간 이상 지났거나 이력이 없으면 무조건 즉시 발행(`Date.now()`). <br> 2) **미래 예약**: 2시간 이내면 `[마지막 시간 + 유저 설정 간격(Jitter 포함)]`으로 예약 잡되, 티스토리 예약 룰 통과를 위해 최소 45분 버퍼 강제 보장. |

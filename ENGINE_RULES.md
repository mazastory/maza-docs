/**
 * ℹ️ 이 문서의 `server/...` 는 maza-studio 리포지토리 기준 경로다.
 *    여기(maza-docs)는 정적 SPA 라 백엔드가 없다. 2026-09-06 에 이 리포에
 *    있던 server/·dist-server/ 사본을 제거했는데(7c41be04), 빌드·배포·의존성
 *    어디에도 들어간 적 없는 옛 사본이었다.
 *    본문 경로 19개는 대조 결과 전부 maza-studio 에 실재한다 — 유효한
 *    경로이니 maza-studio 에서 찾을 것.
 */

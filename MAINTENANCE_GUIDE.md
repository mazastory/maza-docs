...
5. W-05 10800초 값이 server/lib/engine.ts와 extension/background/QueueManager.js에 살아있는지 확인해줘
```
...
WHERE table_schema = 'public' AND table_name LIKE 'ms_%'
ORDER BY table_name;

-- ms_topic_clusters(시리즈) 컬럼 확인
SELECT column_name, data_type, column_default
FROM information_schema.columns
WHERE table_name = 'ms_topic_clusters' ORDER BY ordinal_position;

-- RLS 활성화 여부 확인
SELECT schemaname, tablename, rowsecurity
FROM pg_tables WHERE schemaname = 'public' AND tablename LIKE 'ms_%';
```

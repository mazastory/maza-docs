...
export class FeedbackEngine {
     ...
     */
    static async syncPerformance(siteId) {
        try {
     ...
     * 7-3. 애드센스 승인 성공률 분석
     */
    static async analyzeApprovalSuccess(userId) {
        try {
            ...
            if (!approvedSites || approvedSites.length === 0)
                return;
            const nicheStats = approvedSites.reduce((acc, site) => {
     ...
     */
    static async learnWinningPatterns(siteId) {
        try {
            if (!supabaseAdmin)
                return;
      ...
      ${topPosts.map((p) => `- Title: "${p.title}", Length: ${p.word_count}, CTR: ${p.gsc_ctr}, Intent: ${p.metadata?.validation_v2?.searchIntent?.intent}`).join('\n')}
     ...
     */
    static async getStrategyContext(userId) {

...
export class AuthorityEngine {
     ...
     */
    static async getInternalLinkSuggestions(userId, postId, keyword) {
        try {
            ...
            if (post.cluster_id) {
                const { data: clusterPosts } = await supabaseAdmin
                    ...
                    .limit(3);
                links = (clusterPosts || []).map((p) => ({ title: p.title, url: p.published_url }));
            }
            ...
            if (links.length < 3) {
                ...
                const recentLinks = (recentPosts || []).map((p) => ({ title: p.title, url: p.published_url }));
     ...
     * - 인덱싱 비율 (10%)
     */
    static async calculateTopicalDepth(siteId, clusterIdOrCategory) {
        try {
            // 1. 해당 클러스터 글 통계
            ...
            const avgCTR = posts.reduce((sum, p) => sum + (p.gsc_ctr || 0), 0) / publishedCount;
            const indexedRatio = posts.filter((p) => p.is_indexed).length / publishedCount;
            // 2. 내부 링크 수 집계
            ...
            const score = (Math.min(publishedCount / 10, 1.0) * 0.40) +
     ...
     */
    static async identifyPillarPost(clusterId) {
        try {
            const { data: posts } = await supabaseAdmin
                .from('ms_posts')
     ...
     */
    static async registerInternalLinks(sourcePostId, targetPostIds) {
     ...
     */
    static analyzeInformationDensity(content) {
        try {
            ...
            const densityScore = (Math.min(dataPoints / 5, 1.0) * 30) +

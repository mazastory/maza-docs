...
export function analyzeSEO(content, keyword) {
    ...
    if (content.length < 1200)
        issues.push("본문 분량 부족 (1200자 이상 권장)");
    if (keyword) {
        const count = (content.match(new RegExp(keyword, "gi")) || []).length;
        if (count < 6)
            issues.push(`키워드 밀도 낮음 (현재 ${count}회)`);
    }
    else {
        issues.push("타겟 키워드 설정 누락");
    }
    if (!content.includes("##") && !content.includes("<h2"))
        issues.push("소제목(H2) 구조 누락");
    if (!content.toLowerCase().includes("faq") && !content.includes("자주 묻는 질문"))
        issues.push("FAQ 섹션 누락");
    return issues;
}
// 🔥 자동 개선 루프 (시스템이 만족할 때까지 재작성)
export async function improveSEO(content, keyword) {

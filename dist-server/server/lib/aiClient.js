...
let cachedBudgetStatus = null;
let lastBudgetCheck = 0;
const BUDGET_CHECK_INTERVAL = 1000 * 60 * 10; // 10 minutes
async function checkBudgetGuardrail(options) {
...
export async function callAI(prompt, options = {}) {
    ...
    try {
        ...
        if (Array.isArray(prompt)) {
            ...
            geminiContents = prompt
                ...
                if (p.inlineDataArray) {
                    p.inlineDataArray.forEach((data) => parts.push({ inlineData: data }));
                }
                return {
        ...
        if (isModelQuarantined) {
            const currentIndex = MODEL_CHAIN.indexOf(modelName);
        ...
        if (options.useCache !== false && supabaseAdmin) {
            supabaseAdmin.from('ms_ai_cache').upsert({
                ...
                model: modelName,
                response: text
            }, { onConflict: 'prompt_hash' }).then(({ error: cacheError }) => {
                if (cacheError)
                    console.error(`[AI Client] Cache save failed:`, cacheError.message);
            });
        }
        if (options.rawResponse) {
        ...
        const isAccessOrNotFoundError = error.status === 404 ||
            error.status === 403 ||
...
export async function callAIStream(prompt, options = {}) {
    const rawKeys = process.env.GEMINI_TEXT_API_KEY || process.env.GEMINI_API_KEY || "";
 ...
 */
// 1. 분석 (저렴하고 빠른 처리용)
export async function analyzeAI(prompt) {
...
// 3. 스트리밍 (UX 개선용)
export async function streamAI(prompt, onChunk) {
...
export async function callCheapAI(prompt, options = {}) {
...
export async function callDeepAI(prompt, options = {}) {
    return callAI(prompt, {
 ...
 */
export async function callAgent(prompt, agentName, options = {}) {

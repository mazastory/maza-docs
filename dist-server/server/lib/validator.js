...
export class MazaValidator {
     ...
     * 단순 정적 규칙 기반 (빠름, 안정적)
     */
    static validateStatic(content, keyword, options) {
        const suggestions = [];
        ...
        if (charCount >= wordCountTarget)
            wordCountScore = 20;
        else if (charCount >= wordCountTarget * 0.7)
            wordCountScore = 12;
        else if (charCount >= wordCountTarget * 0.4)
            wordCountScore = 6;
        ...
        const totalScore = keywordInTitleCheck.score +
     ...
     */
    static async validateAI(content, keyword, userId) {
    ...
    static async checkSemanticDuplication(content, keyword, userId) {
        if (!supabaseAdmin)
            return { score: 25, similarity: 0, isDuplicate: false, detail: "DB 연결 없음" };
        const { data: recentPosts } = await supabaseAdmin
            ...
            .limit(5);
        const recentContext = recentPosts?.map((p) => `제목: ${p.title}, 키워드: ${p.keyword}`).join('\n') || "없음";
        const prompt = `
    ...
    static async checkAIFootprint(content) {
    ...
    static async checkSearchIntent(content, keyword) {
        const prompt = `
    ...
    static async checkTopicDepth(content, keyword, userId) {
        const prompt = `
 ...
 */
export function validateContent(content, keyword, options) {
...
export const ValidatorV2 = {
    validate: (content, keyword, userId) => MazaValidator.validateAI(content, keyword, userId)
};

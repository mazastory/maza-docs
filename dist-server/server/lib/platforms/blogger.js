...
export class BloggerAPI {
    static getAuthClient(providerToken) {
        const auth = new google.auth.OAuth2(process.env.GOOGLE_CLIENT_ID, process.env.GOOGLE_CLIENT_SECRET, `${process.env.APP_URL || 'https://mazastudio.kr'}/auth/callback`);
     ...
     */
    static async listBlogs(providerToken) {
        try {
     ...
     * 블로그스팟에 새로운 글을 발행합니다.
     */
    static async post(blogId, providerToken, post) {
        try {

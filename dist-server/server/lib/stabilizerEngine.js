...
export class StabilizerEngine {
     ...
     */
    static async reportFailure(siteId, error) {
        try {
            if (!supabaseAdmin)
                return;
    ...
    static async isSiteActive(siteId) {
        const { data: health } = await supabaseAdmin
            .from('ms_site_health')
            .select('status')
            .eq('site_id', siteId)
     ...
     * 리소스별 사용량 체크 및 차단
     */
    static async checkUsage(userId, resource) {
        try {
     ...
     * 설정 변경 시 스냅샷 저장
     */
    static async takeSnapshot(userId, type, data) {

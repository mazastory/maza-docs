 ...
 */
export const aiWorker = new Worker('generate-post', async (job) => {
    ...
    try {
      ...
      `;
        ...
        if (data.insightBox)
            uiBlocks.push({ type: "insight", text: data.insightBox });
        ...
        if (data.summary)
            uiBlocks.push({ type: "summary_box", text: data.summary });
        if (data.content2) {
        ...
        if (data.experienceNote)
            uiBlocks.push({ type: "experience", text: data.experienceNote });
        if (data.detailedFaqs)
            uiBlocks.push({ type: "faq", items: data.detailedFaqs });
        if (data.outro)
            uiBlocks.push({ type: "conclusion", text: data.outro });
        if (data.hashtags)
            uiBlocks.push({ type: "hashtags", items: data.hashtags });

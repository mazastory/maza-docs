 ...
 */
export const publishWorker = new Worker('publish-post', async (job) => {
    ...
    try {
        ...
        if (!site)
            throw new Error('Site info missing for publication');
        ...
        if (site.platform === 'wordpress') {
            ...
            if (!result.success)
                throw new Error(result.error || 'WordPress API Error');
            ...
            if (!isUsageOk)
                throw new Error('Tistory daily publish limit reached');
            ...
            const result = await BloggerAPI.post(blogger_blog_id, blogger_access_token, {
                ...
                content: post.html_content || post.content,
                isDraft: false,
                labels: post.category ? post.category.split(',').map((c) => c.trim()) : []
            });
            if (!result.success)
                throw new Error(result.error || 'Blogger API Error');
            if (scheduledId) {
                await supabaseAdmin.from('ms_scheduled_posts').update({ status: PostStatus.VERIFYING }).eq('id', scheduledId);
            }
            await supabaseAdmin.from('ms_posts').update({ status: PostStatus.VERIFYING }).eq('id', postId);
...
async function updateStatusSuccess(postId, scheduledId, url) {
    if (!supabaseAdmin)
        return;
    await supabaseAdmin
        .from('ms_posts')

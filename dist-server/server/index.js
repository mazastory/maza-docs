    ...
    try {
      ...
      `)
        ...
        (scheduled || []).forEach((item) => {
            if (!item.ms_posts || !item.ms_sites)
                return;
            ...
            if (engineStatus && engineStatus !== 'ready_to_publish' && !isProcessing)
                return;
            allItems.push({
                id: item.ms_posts.id,
        ...
        // Manual draft items
        (drafts || []).forEach((p) => {
            if (allItems.find(item => item.id === p.id))
                return;
...
app.post('/api/extension/infra-inject', requireAuth, async (req, res) => {
    ...
    if (scDoubleMatch)
        clean_sc = scDoubleMatch[1];
    else if (scSingleMatch)
        clean_sc = scSingleMatch[1];
    ...
    // G-로 시작하지 않으면 무효
    if (clean_ga && !clean_ga.startsWith('G-'))
        clean_ga = '';

...
function scoreUrl(html, keyword, url) {
    const suggestions = [];
    let totalScore = 0;
    // 1. 제목에 키워드 포함 (20점)
    ...
    if (!keywordInTitle)
        suggestions.push(`페이지 제목에 키워드 "${keyword}"를 포함하세요.`);
    // 2. 본문 1,500자 이상 (20점)
    ...
    if (!wordCountPassed)
        suggestions.push(`본문이 ${wordCount}자입니다. 1,500자 이상 권장합니다.`);
    // 3. H2/H3 2개 이상 (15점)
    ...
    if (!headingPassed)
        suggestions.push(`H2/H3 소제목을 2개 이상 사용하세요. (현재: ${headingTotal}개)`);
    // 4. 이미지 1개 이상 (15점)
    ...
    if (!hasImage)
        suggestions.push('이미지를 1개 이상 포함하세요.');
    // 5. 내부 링크 (10점)
    let domain = '';
    try {
    ...
    if (!hasInternalLink)
        suggestions.push('블로그 내 다른 글로 연결되는 내부 링크를 추가하세요.');
    // 6. 메타 디스크립션 (10점)
    ...
    if (!hasMetaDesc)
        suggestions.push('메타 디스크립션을 추가하면 검색 클릭률(CTR)이 높아집니다.');
    // 7. 모바일 뷰포트 (10점)
    const hasViewport = /<meta[^>]*name=["']viewport["']/i.test(html);
    const viewportScore = hasViewport ? 10 : 0;
    totalScore += viewportScore;
    if (!hasViewport)
        suggestions.push('모바일 뷰포트 메타태그를 추가하세요.');
    // 등급 결정
    let grade;
    let gradeLabel;
    if (totalScore >= 70) {
...
router.post('/infra', async (req, res) => {
    try {
        ...
        if (sc_tag) {
            ...
            if (contentMatch)
                cleanScTag = contentMatch[1];
            if (cleanScTag.startsWith('google-site-verification='))
                cleanScTag = cleanScTag.replace('google-site-verification=', '');

...
// ─── 공통 AI 프롬프트 빌더 ───────────────────────────────────────
export function buildGeneratePrompt(keyword, strategy) {
 ...
 * 기본 글 생성 (JSON 응답)
 */
router.post('/', requireAuth, validate(generateSchema), checkDailyLimit, async (req, res) => {
 ...
 */
router.post('/stream', requireAuth, validate(generateSchema), async (req, res) => {
    const { keyword, site_id: reqSiteId } = req.body;
    const user_id = req.userId;
    const traceId = req.headers['x-request-id'];
    const targetKeyword = keyword || req.body.topic || '블로그 정보';
    let trackingPostId = null; // [A-2] 실패 추적용 포스트 ID
    ...
    try {
        ...
        if (!uiBlocks && Object.keys(parsed).length > 0) {
            uiBlocks = [];
            if (parsed.intro)
                uiBlocks.push({ type: "intro", text: parsed.intro });
            if (parsed.image1)
                uiBlocks.push({ type: "image", url: parsed.image1, alt: targetKeyword });
            if (parsed.content1) {
                uiBlocks.push({ type: "heading", text: `${targetKeyword}의 핵심 원리와 이해`, level: 2 });
                uiBlocks.push({ type: "paragraph", text: parsed.content1 });
            }
            if (parsed.insightBox)
                uiBlocks.push({ type: "insight", text: parsed.insightBox });
            if (parsed.image2)
                uiBlocks.push({ type: "image", url: parsed.image2, alt: `${targetKeyword} 상세` });
            if (parsed.summary)
                uiBlocks.push({ type: "summary_box", text: parsed.summary });
            if (parsed.content2) {
                uiBlocks.push({ type: "heading", text: "실전 적용 방법 및 주의사항", level: 2 });
                uiBlocks.push({ type: "paragraph", text: parsed.content2 });
            }
            if (parsed.experienceNote)
                uiBlocks.push({ type: "experience", text: parsed.experienceNote });
            if (parsed.image3)
                uiBlocks.push({ type: "image", url: parsed.image3, alt: `${targetKeyword} 마무리` });
            if (parsed.detailedFaqs && parsed.detailedFaqs.length > 0) {
                uiBlocks.push({ type: "faq", items: parsed.detailedFaqs });
            }
            if (parsed.outro)
                uiBlocks.push({ type: "conclusion", text: parsed.outro });
            if (parsed.hashtags)
                uiBlocks.push({ type: "hashtags", items: parsed.hashtags });
        }
...
4. 수정이 완료된 전체 블록 배열을 유효한 JSON 배열 형태로만 반환하세요.`;
        ...
        try {
            ...
            if (!Array.isArray(revisedBlocks))
 ...
 * Consulting.tsx에서 호출
 */
router.post('/consulting', requireAuth, async (req, res) => {
    try {
 ...
 */
router.post('/recommend-titles', requireAuth, async (req, res) => {
    try {
 ...
 */
router.post('/save-legal', requireAuth, async (req, res) => {
    try {
